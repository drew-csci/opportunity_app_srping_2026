...
from pages.models import Opportunity, OrganizationFollow, Application, Notification, ContactMessage
import json
...
class ContactUsIntegrationTest(TestCase):
    ...
    def test_contact_page_loads_for_student(self):
        self.client.login(username='contact_student', password='Pass123!')
        response = self.client.get(reverse('contact_us'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Contact Us')

    def test_contact_page_loads_for_organization(self):
        self.client.login(username='contact_org', password='Pass123!')
        response = self.client.get(reverse('contact_us'))
        self.assertEqual(response.status_code, 200)
